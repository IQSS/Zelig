...
This guide is designed to get you up and running with Zelig 5.0. For more detailed tutorials see individual model vignettes in the :ref:`userguide`.
...
To install R, go to http://www.r-project.org/ and select the ``CRAN`` option from the left-hand menu. Select a CRAN mirror closest to your current geographic location and follow the instrictuins for donwloading R for Linux, (Mac) OS X, or Windows. 

------------

**Installing Zelig**

Becuase Zelig 5.0 is still in alpha and is not yet on ``CRAN``, it must be downloaded from Github using the ``devtools`` package. Once R is installed, open the terminal and type in the following commands:

.. sourcecode:: r
  
	> install.packages("devtools") # Install devtools package if not already installed
	> library(devtools) # Load devtools
	> install_github('IQSS/Zelig5') # Download latest Zelig 5.0 R source from the IQSS github repo
