...
-  **On Windows**: If you are using the very latest version of R, you
...
-  **On Mac**: If the latest version of Zelig is not yet available at
...
-  **On Mac or Linux systems**: If you get the following warning message
...
that *Windows* R uses the *Linux* directory delimiter. That is, if you
right click and select the “Properties” link on a Windows file, the
slashes are backslashes (:math:`\backslash`), but Windows R uses forward
slashes (/) in directory paths. Thus, the Windows link may be
C:\ :math:`\backslash`\ Program
Files\ :math:`\backslash`\ R\ :math:`\backslash`\ :math:`\backslash`,
...
-  **Windows** users may find old binaries at
   http://gking.harvard.edu/bin/windows/contrib/ and selecting the
   appropriate version of R.

-  **Linux** and **MacOSX** users may find source files at
   http://gking.harvard.edu/src/contrib/
...
send your question to the listserv at ``zelig@lists.gking.harvard.edu``.
...
our installation example, this is ~/.R/library/Zelig/. For Windows users
under R , this is by default C:\ :math:`\backslash`\ Program
Files\ :math:`\backslash`\ R\ :math:`\backslash`\ :math:`\backslash`\ library\ :math:`\backslash`\ Zelig\ :math:`\backslash`.
For Macintosh users, this is ~/Library/R/library/Zelig/.
