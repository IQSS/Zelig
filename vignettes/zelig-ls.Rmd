...
compatibility wrappers see the [Quickstart Vignette](articles/quickstart.html).
...
then you may see the coefficients by using `getcoef`:

```{r}
z.out1$getcoef() 
```

or the variance-covariance matrix with `getvcov`:
...
Or as we saw before using the wrapper:
...
-  From the `zelig()` output object `z.out`, you may extract:
   ...
   -  `getresiduals`: the working residuals in the final iteration of the
      IWLS fit.

   -  `getfitted`: fitted values.
