...
vignette: >
  %\VignetteIndexEntry{zelig-probitgee}
...
Generalized Estimating Equation for Probit Regression with `probit.gee`.
...
```{r Zelig-probitgee1, dev=c("png", "pdf"), eval = TRUE, fig.cap = "Graphs of Quantities of Interest for Probit GEE Model"}
plot(s.out1)
```
...
```{r, eval = TRUE}
x.high <- setx(z.out1, educate = quantile(turnout$educate,
                                          prob = 0.75))
x.low <- setx(z.out1, educate = quantile(turnout$educate,
                                         prob = 0.25))
```
...
```{r Zelig-probitgee2, dev=c("png", "pdf"), eval = TRUE, fig.cap = "Graphs of Quantities of Interest for Probit GEE Model"}
plot(s.out2)
```
