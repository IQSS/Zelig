...
Currently, only the Zelig 5 reference class syntax is available for `arima` 
models:

```{r, eval = FALSE}
<<<<<<< HEAD
z.out <- zelig(Y ~ X1 + X2, model = "arima", order = c(1, 0, 0), data = mydata)
setx(z.out)
sim(z.out) #Currently not implemented
=======
z.out <- zarima$new()
