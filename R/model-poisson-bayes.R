...
#' tune: Metropolis tuning parameter, either a positive scalar or a vector of length kk, where kk is the number of coefficients. The tuning parameter should be set such that the acceptance rate of the Metropolis algorithm is satisfactory (typically between 0.20 and 0.5). The default value is 1.1.
#' @param tune: Metropolis tuning parameter, either a positive scalar or a vector of length kk, where kk is the number of coefficients. The tuning parameter should be set such that the acceptance rate of the Metropolis algorithm is satisfactory (typically between 0.20 and 0.5). The default value is 1.1.
#' @param verbose: default to FALSE. If TRUE, the progress of the sampler (every 10%10%) is printed to the screen.
...
#'   \item \code{tune}: Metropolis tuning parameter, either a positive scalar or a vector of length kk, where kk is the number of coefficients. The tuning parameter should be set such that the acceptance rate of the Metropolis algorithm is satisfactory (typically between 0.20 and 0.5). The default value is 1.1.
